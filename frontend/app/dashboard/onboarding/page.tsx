  ...
  return (
    <div className="w-full px-6 py-6 space-y-6 max-w-full">
      ...
      <div className="bg-[#0a0a0a] border border-white/10 p-6 rounded-2xl w-full">
        ...
        {!extracted ? (
          <label className="border-2 border-dashed border-white/10 hover:border-purple-500/50 transition rounded-xl p-8 flex flex-col items-center justify-center text-center cursor-pointer bg-black/40 block">
            <input type="file" accept=".pdf" onChange={handleFileChange} className="hidden" />
            <div className="p-4 bg-purple-500/10 text-purple-400 rounded-full mb-3 border border-purple-500/20">
              <Upload size={24} />
            </div>
            <span className="text-sm font-medium text-white mb-1">
              {uploading ? "Parsing Manifest with AI..." : "Drop Strata Manifest or Roll Document Here"}
            </span>
            <span className="text-xs text-gray-500">
              {uploading ? "Please wait while we extract lot data" : "Click to browse files locally"}
            </span>
          </label>
        ) : (
          <div className="flex items-center justify-between bg-black/40 border border-white/10 p-4 rounded-xl">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-medium text-white">{file?.name || "Document.pdf"}</p>
                <p className="text-xs text-gray-500">Target Document Loaded & Processed</p>
              </div>
            </div>
      ...
      {/* Bottom Grid: Visible after extraction */}
      {extracted && (
